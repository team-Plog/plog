...
def _rank_spec_candidates(cands: List[str], swagger_ui_url: str) -> List[str]:
    ...
    def score(u: str) -> int:
        s = 0
        if _same_origin(u, swagger_ui_url): s += 10
...
async def analyze_swagger_ui(request: OpenAPISpecRegisterRequest) -> OpenAPISpecModel:
    """
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=30.0) as client:
        ...
        # D) 후보 정리/랭킹
        ranked = _rank_spec_candidates(spec_urls, swagger_ui_url)
